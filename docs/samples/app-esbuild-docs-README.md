...
**app-esbuild** is a production-ready TypeScript application template built with [esbuild](https://esbuild.github.io/), designed to provide an ultra-fast development experience and optimized production builds. This template serves as a solid foundation for building Node.js applications, CLI tools, backend services, and more.

### Why Choose app-esbuild?

- **Blazing Fast Builds**: esbuild compiles TypeScript at speeds 10-100x faster than traditional tools
- **Modern Development Workflow**: Includes hot-reload, type-checking, and comprehensive testing
- **Production Ready**: Pre-configured with Docker, CI/CD, and deployment best practices
- **Developer Experience**: Integrated code quality tools, documentation, and Git workflows
- **Flexible**: Support for multiple build tools (esbuild/tsx/tsc) to suit different needs
...
└── Dockerfile.turbo.prune    # Pruned monorepo build
```
...
```typescript
{
  ...
  bundle: true,
  minify: true,
  external: ['aws-sdk'], // Exclude runtime deps
}
```

#### Container Platforms (Kubernetes, Cloud Run)

Use provided Dockerfiles with:
- Multi-stage builds
- Minimal base images
...
2. **External Dependencies**:
   ```typescript
   {
     external: ['express', 'lodash'], // Don't bundle large deps
   }
   ```
...
For esbuild, configure plugin or use absolute imports.
