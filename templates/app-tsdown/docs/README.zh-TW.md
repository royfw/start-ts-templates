...
**app-tsdown** 是一個使用 [tsdown](https://tsdown.dev/) 構建的生產就緒 TypeScript 應用程式模板。它提供超快的開發體驗和簡化的配置,是啟動 Node.js 應用程式、CLI 工具、後端服務等專案的堅實基礎。

### 為什麼選擇 app-tsdown?

- **零配置** - tsdown 開箱即用,具有合理的預設值
- **極速構建** - 內部使用 esbuild,提供極致速度
- **現代開發工作流** - 包含熱重載、型別檢查和完整測試
- **生產就緒** - 預先配置最佳實踐和部署工具
- **開發者體驗** - 整合程式碼品質工具、文檔和 Git 工作流
- **靈活彈性** - 支援多種構建工具(tsdown/tsx/tsc)以適應不同需求
...
### 1. 使用 tsdown 的超快速構建系統

**tsdown** 提供簡化的構建配置和極致效能:

- **零配置** - 無需複雜的構建配置
- **構建速度** - 由 esbuild 驅動的毫秒級編譯
- **監聽模式** - 檔案變更後即時重建
- **Source Maps** - 完整的除錯支援
- **Bundle 優化** - 自動 tree-shaking 和壓縮
...
1. **入口點** (`main.ts`) - 應用程式啟動
2. **配置** (`configs.ts`) - 集中式配置管理
...
tsdown 預設使用零配置。如需自定義配置,建立 `tsdown.config.ts`:
...
現代 flat config 格式 (`eslint.config.mjs`):
...
**問題**: `tsdown: Build failed`

**解決方案**: 檢查 tsdown 配置並確保所有依賴已安裝。
